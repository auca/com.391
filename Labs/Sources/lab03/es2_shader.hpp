...
public:
    ...
    void compile() final {
        ...
        if (vertex_shader_object == -1) return;

        int fragment_shader_object = _compile_shader(GL_FRAGMENT_SHADER);
        if (fragment_shader_object == -1) return;

        _link_shader(vertex_shader_object, fragment_shader_object);
    }
...
private:
    ...
    int _link_shader(int vertex_shader_object, int fragment_shader_object) {
        ...
        return _program = shader_program;
    }
};
